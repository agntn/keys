...
import Dash, { Dash as NamedDash } from "../../src/blockchains/dash.ts";
import {
  bip39TestVectors,
  dashTestVectors as vector,
...
describe("Dash", () => {
  it("loads through the public registry with SLIP-0044 coin type 5", async () => {
    const chain = await blockchains.dash()();
    expect(chain).toBeInstanceOf(NamedDash);
    expect(chain.name).toBe("dash");
    expect(chain.curve).toBe("secp256k1");
    expect(getBlockchainPath(chain, 0, 1, 3)).toBe("m/44'/5'/0'/1/3");
    expect(() => new Dash({ network: "regtest" })).toThrow("mainnet and testnet only");
  });

  it.each(vector.keys)("writes the node's address for $wif in both encodings", (key) => {
    const chain = new Dash();
  ...
  it("validates the node's P2PKH and P2SH vectors on their network only", () => {
    const mainnet = new Dash();
    const testnet = new Dash({ network: "testnet" });
  ...
  it("signs the node's message_sign vector byte for byte", () => {
    const chain = new Dash();
    const { privateKey, message, address, signature } = vector.signed;
  ...
  it.each(["m/44'/5'/0'/0/0", "m/49'/5'/0'/0/0", "m/84'/5'/0'/0/0"])(
    ...
    (path) => {
      ...
      expect(wallet.address).toMatch(/^X/u);
