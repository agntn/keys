...
TypeScript library providing a unified interface for key generation, address derivation, wallet creation, and message signing across 14 blockchains (Bitcoin, Bitcoin Cash, Bitcoin Gold, Bitcoin SV, Litecoin, Decred, Ethereum, Base, Solana, Stellar, Aptos, Cardano, SUI, TRON). Built entirely on the @noble/@scure audited crypto ecosystem.
...
| Bitcoin keys only  | `src/utils/bitcoin.ts` → `AbstractBitcoinMessageBlockchain`                       | Keys and Core message signing without address formats, as Bitcoin Cash and SV use   |
