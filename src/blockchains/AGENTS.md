...
- **Network params** - Bitcoin Cash keeps its CashAddr prefixes in `NETWORK_PREFIXES`; Bitcoin, Litecoin, Bitcoin Gold, Bitcoin SV, Dash, Dogecoin, Zcash, and Cardano keep separate address parameters for each network in `NETWORK_PARAMS`; TRON uses `0x41` and `T` on mainnet, Shasta, and Nile
