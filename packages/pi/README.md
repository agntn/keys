...
Pi coding agent extension exposing the [`@agntn/keys`](../../README.md) library as 9 agent tools for key generation, mnemonic inspection and recovery, address derivation, validation, signing, and BIP44 paths across 8 blockchains (Bitcoin, Ethereum, Base, Solana, Aptos, TRON, SUI, Cardano).
...
`keys_generate_wallet` returns a plaintext private key, while `keys_derive_wallet` and `keys_sign_message` accept one. The mnemonic tools accept complete or partial phrases and may return equivalent entropy or words allowed by the checksum. Tool arguments and output land in the agent transcript.
