...
Pi coding agent extension exposing the [`@agntn/keys`](../../README.md) library as 6 agent tools for key generation, address derivation, validation, signing, and BIP44 paths across 8 blockchains (Bitcoin, Ethereum, Base, Solana, Aptos, TRON, SUI, Cardano).
...
`keys_generate_wallet` and `keys_sign_message` return the **plaintext private key / signature** in tool output, which lands in the agent transcript.
