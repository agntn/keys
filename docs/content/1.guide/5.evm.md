...
Base looks identical apart from the name. It even keeps coin type 60, because that's what every wallet does for L2s.
...
`signMessage` uses the `personal_sign` preamble, `"\x19Ethereum Signed Message:\n" + length + message`, hashed with Keccak-256 and signed with secp256k1. That's the hash MetaMask and ethers sign, so the same key gives the same `r` and `s` here and there. Default output is 64 bytes of `r||s`, which `verifyMessage` here takes as is.

Everything outside this package wants the `v` too. Ask for it:

```js
ethereumChain.signMessage("hello", privateKey, { recovered: true });
// 65 bytes now, r||s||v with v as 27 or 28, byte for byte what ethers signMessage returns
```

Don't hand the 64-byte one to ethers and hope. `ethers.verifyMessage` reads 64 bytes as an EIP-2098 compact signature, where the top bit of `s` carries the parity, and low-s signatures always have that bit clear. So it doesn't throw, it just answers with the `v = 27` address, wrong half the time. A silent wrong address is worse than an error, and that's the whole reason for the flag.
...
```ts
// src/_blockchains.ts
polygon: lazy("polygon", () => import("./blockchains/polygon.ts")),
```
