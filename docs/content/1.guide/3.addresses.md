...
description: Turn a public key into an address. Pick a format. Check one you were given.
---
...
Every chain does the same thing here: hash the public key, then encode the hash the way the chain likes. What differs is the hash, the encoding, and whether there's a checksum.
...
Bitcoin and Litecoin have five formats, Bitcoin Gold four, Cardano three, Sui uses the argument to name the curve. Bitcoin Cash, Bitcoin SV, Dash, Decred, Dogecoin, Zcash and eCash have one and throw if you ask for another. Ethereum, Base, TRON, Solana, Stellar, and Aptos have one format and ignore the argument.
...
Validation checks the format and the checksum when the format has one. It doesn't touch the network. A valid address with no history is still valid.
...
| Bitcoin Cash | Bitcoin's P2PKH hash behind a version byte, then CashAddr with the `bitcoincash` prefix and a 40-bit checksum |
| Bitcoin Gold | the same hash and encodings, version `0x26` for legacy (`G`), `0x17` for p2sh (`A`), bech32 with `btg`, no taproot |
| Bitcoin SV | Bitcoin's legacy recipe, version `0x00`, base58check, so it starts with `1` |
| Litecoin | the same hash and encodings, version `0x30` for legacy (`L`), `0x32` for p2sh (`M`), bech32 with `ltc` |
| Dash | Bitcoin's legacy recipe, version `0x4c`, base58check, so it starts with `X` |
| Decred | `RIPEMD160(BLAKE256(pubkey))` behind the two byte prefix `07 3f`, base58check with a double BLAKE-256 checksum, so it starts with `Ds` |
| Dogecoin | Bitcoin's legacy recipe, version `0x1e`, base58check, so it starts with `D` |
| Zcash | Bitcoin's legacy recipe behind the two byte version `1c b8`, base58check, so it starts with `t1` |
| eCash | Bitcoin Cash's recipe under the `ecash` prefix, so the same key hash starts with `ecash:q` |
...
| Stellar | version byte `0x30`, the ed25519 public key, a CRC16-XModem checksum, all in base32, so it starts with `G` |
