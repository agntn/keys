...
description: Install the package. Load a chain. Get a wallet out in five lines.
---
...
`@agntn/keys` puts eighteen chains behind one class shape. Same calls, same option object, same wallet type. The chain decides the rules, you decide the key.
...
Node.js 24 or newer. Pure JavaScript all the way down, nothing to compile, and no `node:` import anywhere, so a browser bundle needs no shim. The [Keyspace](/keyspace) explorer is exactly that bundle.
...
| Bitcoin Cash | secp256k1 | Bitcoin's P2PKH hash in CashAddr, starts with `bitcoincash:q` |
| Bitcoin Gold | secp256k1 | four of Bitcoin's five, with `G`, `A` and `btg1q` prefixes, no taproot |
| Bitcoin SV | secp256k1 | legacy P2PKH, the same `1` address Bitcoin gives |
| Litecoin | secp256k1 | the same five, with `L`, `M` and `ltc1` prefixes |
| Dash | secp256k1 | legacy P2PKH, starts with `X` |
| Decred | secp256k1 | legacy ECDSA P2PKH, starts with `Ds` |
| Dogecoin | secp256k1 | legacy P2PKH, starts with `D` |
| Zcash | secp256k1 | transparent P2PKH, starts with `t1` |
| eCash | secp256k1 | Bitcoin's P2PKH hash in CashAddr, starts with `ecash:q` |
...
Each chain has its own page under [Blockchains](/blockchains) with the exact rules and the gotchas. Two of them refuse `deriveHDWallet` on purpose, Decred and Cardano, and their pages say why.
...
Wallets, HD derivation, addresses, signing, BIP44 paths, mnemonics in all ten BIP39 lists, WIF both ways and public key conversion, with the same parameters the library takes. The Pi extension in [`packages/pi`](https://github.com/agntn/keys/tree/main/packages/pi) runs the exact same executors from a checkout. A generated mnemonic comes back with a line saying it's in the transcript now. That line isn't decoration, it's the whole security model: public puzzle material and throwaway keys only.
