...
const wallet = bitcoin.deriveWallet(privateKey, { compressed: true }, "segwit");
wallet.address;
```

This public Electrum test vector gives `bc1q4794m2uuw9jmjszmplfj4wvvr5j272fpnx2cse`. The result identifies `scheme` as `electrum` and `seedType` as `segwit`. The seed is 64 bytes, not a master private key. Standard seeds work too. Inspection also recognizes `old`, `2fa` and `2fa_segwit`, but derivation rejects them, along with `unknown`.
