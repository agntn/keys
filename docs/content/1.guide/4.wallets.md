...
- The mnemonic has to pass the English BIP39 checksum. A typo in word eleven fails before any derivation happens, unless you pass `{ allowInvalidChecksum: true }`: then you get the wallet plus a `warnings` entry, derived from the words exactly as given, nothing repaired. Word count and dictionary are still enforced.
