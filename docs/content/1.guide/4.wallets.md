...
- Bitcoin and Litecoin read the purpose when you don't pass an address type. `m/44'` gives legacy, `m/49'` p2sh, `m/84'` segwit, `m/86'` taproot. An explicit type wins. Bitcoin Cash has only P2PKH, so it gives CashAddr on any purpose. Bitcoin SV gives its `1` address on any purpose too.
