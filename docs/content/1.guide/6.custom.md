...
- `AbstractBitcoinBlockchain` in `src/utils/bitcoin.ts` is what Bitcoin, Litecoin and Bitcoin Gold share: the five address formats, validation, purpose inference on HD paths and Core style message hashing. A subclass brings a `name`, a `bip44`, a `messagePreamble` and a `params` getter with the version bytes and the bech32 prefix. Its keys and Core style message signing live one level down in `AbstractBitcoinMessageBlockchain`, which Bitcoin Cash and Zcash extend because they keep those and write addresses their own way. Bitcoin SV, Dash and Dogecoin go through `AbstractBitcoinP2PKHBlockchain` beside it, where a subclass passes a label and a table of one byte versions to `super()` and gets base58 P2PKH. None of them is exported from the package yet, so they're for drivers inside this repo.
