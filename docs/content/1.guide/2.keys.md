...
Mind what that does to addresses. A legacy Bitcoin address hashes the bytes you give it, so the compressed and the uncompressed encoding of one key are two different addresses. SegWit and the P2SH form take the compressed key only, as BIP143 requires, so an uncompressed key there throws. Keep the `compressed` flag next to the key, the way WIF does.
