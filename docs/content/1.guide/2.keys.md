...
Wallet import format is the private key in base58check with a chain prefix and a compression flag. Bitcoin, Litecoin, Dash, Decred and Dogecoin each have one, and `encodeWIF` and `decodeWIF` speak all five.
...
encodeWIF(privateKey, { chain: "dash" }); // X...
encodeWIF(privateKey, { chain: "dogecoin" }); // Q...
