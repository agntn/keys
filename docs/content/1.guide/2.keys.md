...
icon: i-lucide-key
description: Private keys, public keys, compression, WIF, and the one chain that speaks two curves.
---
...
Mind what that does to addresses. A legacy Bitcoin address hashes the bytes you give it, so the compressed and the uncompressed encoding of one key are two different addresses. Keep the `compressed` flag next to the key, the way WIF does.
