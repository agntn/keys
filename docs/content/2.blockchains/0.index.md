...
navigation:
  title: Overview
description: Eighteen drivers and one class shape. Pick a chain to see its address rules and the traps.
---
...
:::card-group
  ...
  title: Dash
  ---
  secp256k1. Bitcoin's P2PKH behind an `X`, a preamble still called DarkCoin, and no SegWit.
  ::

  ::card
  ---
  icon: i-token-doge
  to: /blockchains/dogecoin
  title: Dogecoin
  ---
  secp256k1. Bitcoin's P2PKH behind a `D`, its own preamble, and no SegWit to wrap anything in.
  ::
  ...
  ---
  icon: i-token-zec
  to: /blockchains/zcash
  title: Zcash
  ---
  secp256k1. The transparent half only: `t1` behind two version bytes, and nothing shielded.
  ::

  ::card
  ---
  icon: i-token-xec
  to: /blockchains/ecash
  title: eCash
  ---
  secp256k1. Bitcoin Cash's CashAddr under `ecash:`, its own preamble, and none of the CashTokens types.
  ::
