...
In @agntn/keys, all EVM chains share a common implementation for key generation, address derivation, and address validation.

## Supported EVM Chains

@agntn/keys currently supports the following EVM chains:
...
@agntn/keys automatically generates addresses with EIP-55 checksums and can validate addresses with or without checksums.
...
@agntn/keys provides address validation for EVM chains that checks both the format and the EIP-55 checksum:
