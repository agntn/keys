...
In the context of @agntn/keys, a "wallet" combines keys (private and public) with a blockchain address. This provides a complete solution for interacting with a blockchain.
...
@agntn/keys provides a convenient way to generate a complete wallet in one step:
...
While @agntn/keys currently focuses on key and address generation, future versions may include transaction creation and signing functionality.

## Importing Existing Keys

If you already have a private key, derive the public key and address in one call:
...
Key options and address types work the same way as with `generateWallet`:

```js
const segwitWallet = bitcoinChain.deriveWallet(privateKey, {}, "segwit");
```

## Deriving from a Mnemonic

`deriveHDWallet` walks a BIP39 mnemonic down a derivation path and returns the wallet at the end of it:

```js
const mnemonic =
  "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";

bitcoinChain.deriveHDWallet(mnemonic, "m/84'/0'/0'/0/0").address; // bc1q...
...
::warning
@agntn/keys is primarily a tool for interacting with blockchain protocols. It does not implement secure key storage. For production applications, consider using additional security measures.
::
