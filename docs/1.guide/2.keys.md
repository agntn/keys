...
All blockchains in `@agntn/keys` share the same private key generator. Each key contains 32 bytes (256 bits) of cryptographically secure random data represented by 64 hexadecimal characters.
...
Keys in @agntn/keys are represented as hexadecimal strings:
...
@agntn/keys uses a structured format for representing key pairs:
...
Under the hood, @agntn/keys uses the following libraries for cryptographic operations:
